...
export async function GET(request: Request, { params }: { params: { applicationId: string } }) {
  ...
  try {
    ...
    const certificates = await db
      .select({
        certificate_id: expertApplicationCertificatesTable.certificate_id,
      ...
      .where(eq(expertApplicationCertificatesTable.application_id, applicationId));
    ...
    const categories = await db
      ...
      .innerJoin(
        categoriesTable,
        eq(expertApplicationCategoriesTable.category_id, categoriesTable.category_id)
      )
      .where(eq(expertApplicationCategoriesTable.application_id, applicationId));
